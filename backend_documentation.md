...
- **CORS:** PostgREST в Supabase отдаёт `Access-Control-Allow-Origin: *` и настройки ограничения origin не имеет (аудит F-12) — фронтенд с любого домена работает без настройки; реальный контроль доступа — RLS (данные) и Auth → URL Configuration (Site URL + allowlist Redirect URLs — куда разрешено возвращать после OAuth).
...
**Мини-кейс 3. `42P13: cannot change return type of existing function`.** Первая версия миграции 2 делала `create or replace function` — PostgreSQL не позволяет так менять тип результата (`documents` → `jsonb`). Подсказка сервера (`HINT: Use DROP FUNCTION … first`) дала исправление в одну строку; а потому что миграция обёрнута в `begin … commit`, неудачный запуск не оставил половинчатого состояния — повторный прогон прошёл чисто. Урок: ошибки PostgreSQL читать до конца — HINT часто и есть готовое решение.

Все случаи — прямое продолжение вывода из ДЗ-1 и ДЗ-4: зелёный прогон (тестов, миграции) не равен «работает»; проверять нужно данные и побочные эффекты, причём другим инструментом, чем тот, что породил результат.
...
**End-to-end в интерфейсе** ([scripts/e2e-sign.mjs](scripts/e2e-sign.mjs), puppeteer на живом бэкенде): вход → карточка «ГетБлоггер» → «Утвердить и подписать» → диалог подтверждения → «Подтвердить» → статус «Подписан» (строка в `sign_attempts`); затем «Крипто-Тест» (Компания В, роль оператор) → та же цепочка → в карточке ошибка «Подписание доступно только роли «Подписант»», статус не изменился. **Изоляция данных:** два пользователя видят по 6 своих документов, пересечение множеств ID — 0.

Живые скриншоты приложения на Supabase (каталог [docs/screenshots](docs/screenshots)): `live-login.png`, `live-desktop.png`, `live-mobile.png`, `live-sign-dialog.png`, `live-signed.png`, `live-sign-denied.png`.

**Тестовые учётные записи для проверяющего** передаются вне репозитория (см. `security_audit.md`, F-03) — либо зарегистрируйте нового пользователя: демо-набор создаётся автоматически.
