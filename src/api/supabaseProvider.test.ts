...
function fakeClient(opts: { rows?: unknown[]; rpcData?: unknown; rpcError?: { message: string } | null }) {
  const query = {
    select: () => query,
...
describe('supabaseProvider', () => {
  it('маппит строки PostgREST в DocumentItem (организация из join, сумма → число)', async () => {
    const p = createSupabaseProvider(
      fakeClient({
        rows: [{ id: '1', counterparty: 'ООО «Тест»', title: 'УПД № 1', kind: 'УПД', sum: '100.00',
                 received_at: '2026-09-16T10:00:00Z', status: 'info', unread: true, organizations: { name: 'Компания А' } }],
      }),
    )
  ...
  it('sign: бизнес-отказ {ok:false} превращается в ошибку с текстом от сервера (негативный)', async () => {
    const p = createSupabaseProvider(fakeClient({ rpcData: { ok: false, error: 'Подписание доступно только роли «Подписант»' } }))
    await expect(p.sign('x')).rejects.toThrow('только роли')
  })

  it('sign: транспортная ошибка PostgREST тоже становится исключением (негативный)', async () => {
    const p = createSupabaseProvider(fakeClient({ rpcError: { message: 'JWT expired' } }))
    await expect(p.sign('x')).rejects.toThrow('JWT expired')
  })
})
