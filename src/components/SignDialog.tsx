    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <dl className="mt-4 space-y-2 text-sm">
          <div className="flex justify-between gap-4"><dt className="text-slate-500">Документ</dt><dd className="font-semibold text-right">{doc.title}</dd></div>
          <div className="flex justify-between gap-4"><dt className="text-slate-500">Контрагент</dt><dd className="font-semibold text-right">{doc.counterparty}</dd></div>
          <div className="flex justify-between gap-4"><dt className="text-slate-500">Сумма (без НДС)</dt><dd className="font-semibold">{formatSum(doc.sum)}</dd></div>
          <div className="flex justify-between gap-4"><dt className="text-slate-500">Подписант</dt><dd className="font-semibold text-right">{doc.org}, сертификат ящика</dd></div>
        </dl>
