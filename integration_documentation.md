...
**Полный вход с реальным Google-аккаунтом проверен 22.09.2026:** после согласия на экране Google пользователь попал в ленту, в шапке отобразились имя из Google-профиля (`user_metadata.full_name`) и email; для нового OAuth-пользователя сработал триггер `handle_new_user` — выдан свежий демо-набор (3 организации, 6 документов). Все три проверки Шага 3 (успешный вход, обработка ошибок, получение данных пользователя) пройдены.
